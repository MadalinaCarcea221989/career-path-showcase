          ...
          {projects.map((project, index) => (
            <Card key={index} className="group overflow-hidden bg-gradient-to-br from-indigo-900/40 via-purple-900/30 to-violet-900/40 backdrop-filter backdrop-blur-xl border-4 border-gradient-to-r from-indigo-400/60 via-purple-400/70 to-cyan-400/60 hover:border-indigo-300/80 rounded-2xl shadow-2xl hover:shadow-indigo-500/30 transition-all duration-500 hover:-translate-y-3 hover:scale-[1.02]">
              ...
              <CardHeader className="pb-4 pt-6">
                <CardTitle className="text-xl font-bold group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-indigo-300 group-hover:to-purple-300 group-hover:bg-clip-text transition-all duration-500 flex items-center gap-3 high-contrast-text">
                  {project.icon}
                  {project.title}
                </CardTitle>
                <CardDescription className="text-base leading-relaxed high-contrast-text font-medium">
                  {project.description}
                </CardDescription>
              </CardHeader>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.technologies.map((tech, techIndex) => (
                    <Badge 
                      key={techIndex} 
                      variant="secondary" 
                      className="text-xs px-3 py-1.5 bg-gradient-to-r from-indigo-500/40 to-purple-500/40 border-2 border-indigo-300/50 text-slate-100 hover:from-indigo-500/50 hover:to-purple-500/50 hover:border-indigo-300/70 transition-all duration-300 font-semibold"
                    >
                      {tech}
                    </Badge>
                  ))}
                ...
                <div className="flex gap-4">
                  {project.liveUrl && (
                    <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="flex-1">
                      <Button size="sm" className="futuristic-button flex items-center justify-center gap-2 w-full relative z-10 py-2.5 font-semibold">
                  ...
                  {project.githubUrl && (
                    <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="flex-1">
                      <Button variant="outline" size="sm" className="flex items-center justify-center gap-2 bg-black/90 border-2 border-black hover:bg-black text-white hover:text-white transition-all duration-300 w-full py-2.5 font-semibold hover:border-gray-700">
